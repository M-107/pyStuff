...
def align(file):
    ...
    peaks, peaks_info = find_peaks(y[:int(len(y)*1/3)], width = 5, height = 1000, prominence = 200)    
    last_dif = 10
    closest = 0
    ...
    for m in range(0, len(x_new)):
        sci_form = "{:e}".format(x_new[m])
        num = f"{sci_form[:-4].ljust(16, '0')}"
        exp = f"E+00{sci_form[-2:]}"
        new_lines.append(f" {num}{exp}  {y[m]}")    
...
# Finds a baseline of a 1d array
...
# Pase one is around 10.1 and phase two is around 12
def get_peaks(x, y):
    phase_1_max = 0
    phase_2_max = 0
    peak_1_coords = []
    ...
    phase_1_percent = round(phase_1_max/(phase_1_max + phase_2_max)*100, 1)
    phase_2_percent = round(phase_2_max/(phase_1_max + phase_2_max)*100, 1)
...
def graph_single(file_path, name, x, y, y_no_air, y_base, crystalline_percentage, phase_1, phase_2, peak_1, peak_2):
    sns.set_theme()
    ...
    plt.plot([peak_1[0], peak_1[0]], [0, peak_1[1]], "r-")
    plt.plot([peak_2[0], peak_2[0]], [0, peak_2[1]], "r-")
...
# Maximum ammount of files in group is five atm, but the only limiting factor is the color gradient
# So it can be easily increased
def graph_group(sublist, name):
    i = sublist
    cols = ["#0d0745", "#130d6f", "#5e72eb", "#ff9190", "#fec195"]
    sns.set_theme()
    sns.set_style("whitegrid")
    fig, ax = plt.subplots()
    ax.set_title(f"{name}")
...
def graph_halftime(sublist, name):
    ...
    if len(times) > 1 and (phases_1[0]-phases_2[0])*(phases_1[-1]-phases_2[-1]) < 0:
...
for i in files_grouped:
    ...
    for j in paths:
        ...
        phase_1, phase_2, peak_1_coords, peak_2_coords = get_peaks(x, y_no_air)
        j.append(crystalline_percentage)
        j.append(phase_1)
        j.append(phase_2)
        graph_single(file, title_name, x, y, y_no_air, y_base, crystalline_percentage, phase_1, phase_2, peak_1_coords, peak_2_coords)
    print("Individual group analyzed, making combined and halftime graphs")
    graph_group(paths, name)
